...
- [x] **DONE** Split health-check persistence out of `OperationsStore` into `HealthOperationsStore`. Tracking: #89
- [x] **DONE** Split backup persistence out of `OperationsStore` into `BackupOperationsStore` and remove the empty aggregate store. Tracking: #91
- [x] **DONE** Keep transaction boundaries explicit across all focused operations stores.
...
Verification evidence for the health operations slice: PR #90, GitHub Actions **Verify .NET application** runs #265 and #266 plus post-merge main run #267 — restore, formatting, Release build, all .NET/PostgreSQL tests including health-target creation, health-check history/current-state convergence, enable/disable behavior and the health persistence ownership boundary, production image builds, PostgreSQL/Caddy UI startup, authenticated page smoke, Chromium UI/UX/localization acceptance, SQLite migration CLI and bundled Caddy module verification all passed. `HealthOperationsStore` owns the complete health persistence surface, and `RecordHealthCheckAsync` keeps the health-check history insert and target-state update inside one explicit transaction.

Verification evidence for the backup operations slice: PR #92, GitHub Actions **Verify .NET application** run #268 — restore, formatting, Release build, all .NET/PostgreSQL tests including backup artifact persistence, newest-first ordering, manifest-object validation and the legacy aggregate-store absence guard, production image builds, PostgreSQL/Caddy UI startup, authenticated page smoke, Chromium UI/UX/localization acceptance, SQLite migration CLI and bundled Caddy module verification all passed. `BackupOperationsStore` owns the complete backup persistence surface and the now-empty `OperationsStore` aggregate was removed entirely.

Operations transaction boundaries remain explicit in the focused owners: DDNS claiming uses a local `ReadCommitted` transaction with `FOR UPDATE ... SKIP LOCKED`; scheduled-job claiming uses the same explicit claim pattern and job-run completion updates run and job state atomically; health-check recording atomically inserts history and updates current target state. Notification and backup persistence contain only single-step writes and therefore use the focused relational plumbing helper without artificial transaction wrappers.
