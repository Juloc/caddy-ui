...
def range_nav(path: str, selected: str, filters: AnalyticsFilters, extra: dict[str, str] | None = None) -> str:
    ...
    for value in ("15m", "1h", "6h", "24h", "7d", "30d", "1y"):
        items.append(f'<a class="range-chip {"active" if value == selected else ""}" href="{path}?{q({**base, "range": value})}">{value}</a>')
...
def tabs(tab: str, query: dict[str, str]) -> str:
    ...
    return '<div class="tabs">' + "".join(f'<a class="{"active" if tab == key else ""}" href="/analytics?{q({**query, "tab": key})}">{label}</a>' for key, label in values) + "</div>"
...
def filter_form(filters: AnalyticsFilters, dimensions: dict[str, list[str]], range_name: str, tab: str, action: str) -> str:
    ...
    reset_target = action + ("&" if "?" in action else "?") + "range=24h"
    return f"""
<form method="get" action="{views.e(action)}" class="analytics-filter-grid" data-analytics-filters>
  ...
  <div class="filter-actions"><button class="primary" type="submit">Apply filters</button><a class="button" href="{views.e(reset_target)}">Reset</a></div>
</form>
"""
...
def chart(title: str, series: list[dict[str, Any]], metric_name: str, unit: str, drill_query: dict[str, str]) -> str:
    ...
    return f'<section class="chart-section span-6"><div class="panel-header"><div><h2>{views.e(title)}</h2><div class="muted">Select a point to open matching logs</div></div></div><div class="analytics-chart" data-chart-series="{views.e(payload)}" data-chart-metric="{views.e(metric_name)}" data-chart-unit="{views.e(unit)}" data-chart-drill="{views.e("/logs?" + q(drill_query))}"><div class="empty">No chart data in this range.</div></div></section>'
...
def analytics_page(session: sqlite3.Row, csrf: str, tab: str, range_name: str, filters: AnalyticsFilters, summary: dict[str, Any], series: list[dict[str, Any]], top_hosts: list[tuple[str, int, float]], top_endpoints: list[tuple[str, int, float]], slow_endpoints: list[tuple[str, int, float]], top_clients: list[tuple[str, int, float]], dimensions: dict[str, list[str]], saved: Iterable[sqlite3.Row], message: str = "", error: str = "") -> bytes:
    ...
    requests = safe_int(summary.get("requests")); errors_4xx = safe_int(summary.get("errors_4xx")); errors_5xx = safe_int(summary.get("errors_5xx"))
    error_rate = (errors_4xx + errors_5xx) / requests * 100 if requests else 0
    cards = "".join((metric("Requests", f"{requests:,}", "Selected range", "/logs?" + q(base)), metric("Average", format_ms(summary.get("avg_ms")), "Response time", "/logs?" + q({**base, "sort": "slow"})), metric("P95", format_ms(summary.get("p95_ms")), f"P99 {format_ms(summary.get('p99_ms'))}"), metric("4xx / 5xx", f"{errors_4xx:,} / {errors_5xx:,}", f"{error_rate:.1f}% errors", "/logs?" + q({**base, "errors": "1"})), metric("Traffic", format_bytes(summary.get("bytes_sent")), "Response bytes"), metric("P50", format_ms(summary.get("p50_ms")), "Median response time"), metric("Maximum", format_ms(summary.get("max_ms")), "Slowest request")))
    if tab == "performance":
    ...
    body = f'<div class="commandbar analytics-commandbar">{range_nav("/analytics",range_name,filters,{"tab":tab})}<div class="commands"><a class="button" href="/logs?{q(base)}">Open logs</a></div></div>{tabs(tab,base)}<details class="filter-panel" {"open" if filters.as_query() else ""}><summary>Filters</summary>{filter_form(filters,dimensions,range_name,tab,"/analytics")}</details><div class="saved-view-bar"><label>Saved view<select data-saved-view><option value="">Choose view</option>{saved_options}</select></label><form method="post" action="/saved-views/save" class="inline-form"><input type="hidden" name="csrf" value="{views.e(csrf)}"><input type="hidden" name="kind" value="analytics"><input type="hidden" name="query" value="{views.e(json.dumps(base,separators=(",",":")))}"><input name="name" placeholder="View name" required maxlength="80"><button type="submit">Save view</button></form></div><div class="grid metrics-grid">{cards}</div>{content}<div class="custom-range-popover" data-custom-range-panel hidden><form method="get" action="/analytics" class="form-grid"><input type="hidden" name="range" value="custom"><input type="hidden" name="tab" value="{views.e(tab)}"><label>Start<input type="datetime-local" name="start" required></label><label>End<input type="datetime-local" name="end"></label><button class="primary" type="submit">Apply custom range</button></form></div>'
...
def filter_chips(filters: AnalyticsFilters, range_name: str) -> str:
    values = {"range": range_name, **filters.as_query()}; chips = []
    for key, value in values.items():
        if not value or (key == "range" and value == "24h"): continue
...
def logs_page(session: sqlite3.Row, csrf: str, rows: Iterable[sqlite3.Row], filters: AnalyticsFilters, range_name: str, dimensions: dict[str, list[str]], total: int, page: int, saved: Iterable[sqlite3.Row], is_admin: bool, message: str = "", error: str = "") -> bytes:
    ...
    for row in rows:
        ...
        ip_target = f'/analytics/client?{q({"ip":row["remote_ip"],"range":range_name})}' if row["remote_ip"] else ""; ip_cell = f'<a href="{views.e(ip_target)}">{views.e(row["remote_ip"])}</a>' if ip_target else "—"
    ...
    if not row_html: row_html.append('<tr><td colspan="10" class="empty">No requests match these filters.</td></tr>')
    saved_options = "".join(f'<option value="{views.e(row["query_json"])}">{views.e(row["name"])}</option>' for row in saved)
    ...
    pages = max(1, (total + 199) // 200); pagination = f'<div class="pagination"><span>{total:,} matching requests · Page {page} of {pages}</span>'
